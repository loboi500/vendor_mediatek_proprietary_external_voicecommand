...
int AudiocheckAndCreateDirectory(const char *pC)
{
    ...
    while (*pC) {
        if ((0 <= i) && (i < PATH_MAX)) {
            ...
            ALOGE("AudioDumpPCM: index i = %d out of bound!\n", i);
            return -1;
        }
    }
    return 0;
...
AudioStream::AudioStream(audio_source_t inputSource, uint32_t sampleRate, uint32_t channelCount, audio_session_t session_id, bool isHotwordTraining) :
    m_bStarted(false),
    ...
    mMaxAmplitude_L(0),
    mMaxAmplitude_R(0),
    m_ChannelMask((audio_channel_mask_t)0)
{
    if (isHotwordTraining) {
    ...
    ALOGD("inputSource = %d, session_id = %d, sampleRate: = %d, channelCount = %d. isHotwordTraining = %d", inputSource, session_id, sampleRate, channelCount, isHotwordTraining);
    ...
    bool isInputSourceValid = true;
    if (isHotwordTraining) {
        if (inputSource != AUDIO_SOURCE_HOTWORD) {
            isInputSourceValid = false;
        }
    } else {
    ...
    attributionSource.packageName = std::string("com.mediatek.voicecommand");
    m_pAudioRecord = new AudioRecord(m_InputSource,            // inputSource
                                     m_SampleRate,             // sampleRate
                                     AUDIO_FORMAT_PCM_16_BIT,  // format
                                     m_ChannelMask,            // channelMask
                                     attributionSource,        // opPackageName
                                     0,                        // frameCount
                                     NULL,                     // cbf
                                     NULL,                     // user
                                     0,                        // noticationFrames
...
status_t AudioStream::openPCMDataFile()
{
    ...
    if (bflag) {
        ...
        ret = sprintf(string_time, "%.2lu_%.2lu_%.2lu_%.3lu",
                (now.tv_sec / 3600) % (24),
                (now.tv_sec / 60) % (60),
                (now.tv_sec % 60),
                (now.tv_usec / 1000));
        if (ret < 0) {
            ALOGE("%s(), string_time sprintf fail! ret = %s", __func__, ret);
        }

        ret = sprintf(filename, "%s/%s_%s",
                DUMP_PCM_DATA_PATH, string_time, filename_ul1);
        if (ret < 0) {
            ALOGE("%s(), filename_ul1 sprintf fail! ret = %s", __func__, ret);
        }
        ...
        if (ret < 0) {
            ALOGE("%s(), filename_ul2 sprintf fail! ret = %s", __func__, ret);
        }
        AudiocheckAndCreateDirectory(filename);
        ALOGI("%s(), filename = %s", __func__, filename);
        ...
        if (ret < 0) {
            ALOGE("%s(), filename_ul_raw sprintf fail! ret = %s", __func__, ret);
        }
        AudiocheckAndCreateDirectory(filename);
        ALOGI("%s(), filename = %s", __func__, filename);
    ...
    //if (1) {
        ret = sprintf(filename, "%s%s", m_BackupFilePath, filename_update_ul);
        if (ret < 0) {
            ALOGE("%s(), filename_update_ul sprintf fail! ret = %s", __func__, ret);
        }
        AudiocheckAndCreateDirectory(filename);
        ALOGI("%s(), filename = %s", __func__, filename);
...
status_t AudioStream::closePCMDataFile()
{
    int ret = 0;
    if (m_fp_uplink1)
        ret = fclose(m_fp_uplink1);
    if (ret != 0) {
        ALOGE("%s(), fclose m_fp_uplink1 fail, ret = %d", __func__, ret);
        return ret;
    }
    m_fp_uplink1 = NULL;

    if (m_fp_uplink2)
        ret = fclose(m_fp_uplink2);
    if (ret != 0) {
        ALOGE("%s(), fclose m_fp_uplink2 fail, ret = %d", __func__, ret);
        return ret;
    }
    m_fp_uplink2 = NULL;
    ...
    if (m_fp_UL)
        ret = fclose(m_fp_UL);
    if (ret != 0) {
        ALOGE("%s(), fclose m_fp_UL fail, ret = %d", __func__, ret);
        return ret;
    }
    m_fp_UL = NULL;

    if (m_fp_BackupUL)
        ret = fclose(m_fp_BackupUL);
    if (ret != 0) {
        ALOGE("%s(), fclose m_fp_BackupUL fail, ret = %d", __func__, ret);
        return ret;
    }
    m_fp_BackupUL = NULL;
...
status_t AudioStream::dumpPCMData(const char* pStr, void * buffer, int count)
{
    ...
    if (fp != NULL) {
        size_t writeCount = 1;
        size_t writeCountRet = fwrite(buffer, count, writeCount, fp);
        if (writeCountRet < writeCount) {
            ALOGE("%s(), write bugger fail! writeCountRet(%d) < writeCount(%d)",
                    __func__, (unsigned int)writeCountRet, (int)writeCount);
        }
    }
    return OK;
}
...
status_t AudioStream::start()
{
    ...
    if (err != OK) {
        ALOGE("start audio source fail, err = %d", err);
        closePCMDataFile();
        return err;
    }
