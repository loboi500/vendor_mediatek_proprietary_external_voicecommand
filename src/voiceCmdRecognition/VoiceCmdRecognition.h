...
#include "voiceunlock2/include/VowEngine_Trainer.h"
#include "voiceunlock2/include/VowEngine_Trainer_CppUtil.h"
using namespace VOWE_Trainer;
...
struct VOW_FEATURE_OPTION {
    bool isDualMicSupport;
    bool isNormalRecordEnroll;
};
...
public:
    ...
    status_t getUtteranceSize(uint32_t *size);
    status_t getUtterance(void *buffer, uint32_t size, uint32_t *writeSize);
    ...
    void getUtterancePcm(void);  // allocate m_UtterancePcm, should be called paired with releaseUtterancePcm
    void releaseUtterancePcm(void);

    bool saveRetrainPcm(void);

    bool getTrainingThreadStarted(void);
    bool getCaptureThreadStarted(void);
    bool setTrainingThreadStarted(bool started);
    bool setCaptureThreadStarted(bool started);

    sp<AudioStream> m_pAudioStream;
    pthread_mutex_t m_trainingThreadStartedMutex;
    pthread_mutex_t m_captureThreadStartedMutex;
    ...
    bool m_ContinueVoiceTraining;
    bool m_bIsTrainingThreadStarted;
    bool m_bIsCaptureThreadStarted;
    int m_PatternFd;
...
private:
    ...
    bool isFeatureOptionEnabled(char *option);
    void getFeatureOption();
    ...
    VOW_FEATURE_OPTION m_VowFeatureOption;

    uint32_t m_UtterancePcmSize;  // unit: byte
    uint8_t *m_UtterancePcm;
};
#endif
