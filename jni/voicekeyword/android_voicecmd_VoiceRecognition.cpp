...
static sp<VoiceCmdRecognition> getVoiceCmdRecognition(JNIEnv* env, jobject thiz)
{
    ...
    //ALOGD("[JNIVOICECMD]@%s,@%d, p = %p",__FUNCTION__,__LINE__, p);
    ...
    if (exception == NULL) {  // Don't throw exception. Instead, send an event.
        ...
        if (opStatus == (status_t) INVALID_OPERATION) {
            ...
            if (strlen(message) > 230) {
                ...
                int ret = sprintf(msg, "%s: status=0x%X", message, opStatus);
                if (ret < 0) {
                    ALOGE("%s(), sprintf fail, ret = %d", __func__, ret);
                }
...
static jint voice_cmd_getUtteranceSize(JNIEnv *env, jobject thiz)
{
    LOGDEBUG_JNI_VOICECMD
    ...
    uint32_t utteranceSize;
    process_VoiceCmdRecognition_call(env, thiz, vchandler->getUtteranceSize(&utteranceSize), NULL, NULL);
    ALOGD("%s(): utteranceSize in byte = %d", __FUNCTION__, utteranceSize);
    return (jint)utteranceSize;
}

static jint voice_cmd_getUtterance(JNIEnv *env, jobject thiz, jobject jBuffer)
{
    LOGDEBUG_JNI_VOICECMD
    ...
    if (capacity == -1) {
        ...
        ALOGE("capacity == -1, can't get utterance");
        jniThrowException(env, "java/lang/RuntimeException", "Buffer direct access is not supported");
        return 0;
    }
    ALOGD("capacity = %ld", capacity);

    jbyte *nativeFromJavaBuf = (jbyte *) env->GetDirectBufferAddress(jBuffer);
    if (nativeFromJavaBuf == NULL) {
        ALOGE("nativeFromJavaBuf == NULL, can't get utterance");
        jniThrowException(env, "java/lang/RuntimeException", "Buffer direct access is not supported");
        return 0;
    }
    // write utterance PCM to the buffer passed from Java
    uint32_t writeSize = 0;
    process_VoiceCmdRecognition_call(env, thiz, vchandler->getUtterance(nativeFromJavaBuf, capacity, &writeSize), NULL, NULL);
    return (jint)writeSize;
}
...
static JNINativeMethod gMethods[] = {
    ...
    {"getUtteranceSize", "()I", (void *)voice_cmd_getUtteranceSize},
    {"getUtterance", "(Ljava/nio/ByteBuffer;)I", (void *)voice_cmd_getUtterance},
